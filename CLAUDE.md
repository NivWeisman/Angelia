...
5. **SSH host strings are opaque.** Pass them straight to `ssh`. Never parse them yourself — `~/.ssh/config` aliases, jump hosts, `user@host:port`, key files, etc. all work transparently if you don't touch the string.
   - **Route every remote command through `angelia-client--login-wrap`** (lisp/angelia-client-deploy.el), never a hardcoded `bash --login -c`. `ssh host CMD` runs CMD under the remote's *default* login shell; that shell may be csh/tcsh, which won't load `~/.login`/`/etc/csh.login` (path_helper → PATH) on a bare `-c`. `login-wrap` detects the shell family (`echo $SHELL`, cached) and either sources the csh login files then `exec bash`, or uses `bash --login`. Wrapped commands must contain **no single quotes** (csh single-quoting is literal).

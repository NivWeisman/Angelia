...
- **Server (stderr only):** startup banner with version hash + Emacs version + PID; every raw stdin frame (truncated 500); dispatch (method, handler); handler entry/exit + elapsed ms; every raw stdout frame (truncated); every flush; full backtraces on error.
- **Client (`*angelia-client-debug*` buffer):** connection attempt + deploy phase + SSH process start (full command line); sentinel events; every RPC request (id, method, truncated params); every RPC response (id, result/error truncated); file handler dispatch (op, path); per-request round-trip timing.
